...
export default function EditNewsPage({
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const itemRes = await fetch(`${API}/api/news/${id}`);
        if (!itemRes.ok) { setNotFound(true); setLoading(false); return; }

        const item: NewsItem = await itemRes.json();

        setHeading(item.news_heading || "");
        setContent(item.news_content || "");
        setTag1(item.tag_1 || "");
        setTag2(item.tag_2 || "");
        setTag3(item.tag_3 || "");
        setCustomUrl(item.custom_url || "");
        setAltTag(item.alt_tag_featured || "");
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-full bg-[#f8fafc]">
        <AdminHeader title="Modifying Record" subtitle="Loading article details..." />
  ...
  if (notFound) {
    return (
      <div className="flex flex-col h-full bg-[#f8fafc]">
        <AdminHeader title="Article Missing" />
        <div className="flex-1 flex flex-col items-center justify-center p-20 text-center">
          <p className="text-[18px] text-primary/30 font-bold mb-6">Article record not found in database.</p>
          <Link href="/admin/news" className="bg-primary text-white px-8 py-4 rounded-xl font-bold no-underline">
            ← Browse News
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#f8fafc]">
      <AdminHeader title="Edit News Article" subtitle={`Modifying: ${heading}`} />
      ...
      <main className="p-8 transition-all duration-500 animate-in fade-in slide-in-from-bottom-4 overflow-x-hidden">
        <div className="flex items-center gap-4 mb-6">
          ...
          <p className="text-[13px] font-bold text-slate-500 uppercase tracking-widest">Newsroom / Edit Details</p>
        </div>
            ...
            <div className="bg-white rounded-[24px] p-8 shadow-[0_4px_40px_rgba(19,47,70,0.03)] border border-primary/10">
              ...
              <div className="grid grid-cols-3 gap-5">
                {[
                  ...
                  <div key={label}>
                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-600 mb-1.5">{label}</label>
                    <input
                      type="text"
                      value={val}
                      onChange={(e) => set(e.target.value)}
                      className="w-full bg-[#f8fafc] border border-primary/10 rounded-xl px-4 py-2 text-[14px] font-bold text-primary focus:bg-white focus:border-gold outline-none transition-all"
                    />
                  </div>
                ))}
              </div>
          ...
          <div className="col-span-4 space-y-6">
            ...
            <div className="bg-primary rounded-[20px] p-6 text-white shadow-xl">
              ...
              <div className="space-y-4 mb-6">
                 <div>
                    <label className="block text-[10px] font-bold uppercase tracking-widest text-white/40 mb-1.5">URL Identifier</label>
                    <input
                      type="text"
                      value={customUrl}
            ...
            <div className="bg-white rounded-[20px] p-6 shadow-[0_4px_40px_rgba(19,47,70,0.03)] border border-primary/10">
              ...
              <div className="space-y-4">
                <div className="aspect-video bg-[#f8fafc] rounded-xl border-2 border-dashed border-primary/10 flex flex-col items-center justify-center p-4 relative overflow-hidden group">
                   {displayImage ? (
                      ...
                      <div className="text-center flex flex-col items-center">
                         ...
                         <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">Upload Headline Image</p>
                      </div>
                   )}
                   <input
                     ref={fileInputRef}
                ...
                <div>
                   <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-600 mb-1.5">Accessibility (Alt Text)</label>
                   <input
                     type="text"
                     value={altTag}
                     onChange={(e) => setAltTag(e.target.value)}
                     className="w-full bg-[#f8fafc] border border-primary/10 rounded-xl px-4 py-2 text-[13px] font-bold text-primary focus:bg-white focus:border-gold outline-none transition-all"
                   />
                </div>
              </div>
            </div>
